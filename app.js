...
/* ✅ Sinn-wahrende Bereinigung:
   - Entfernt GANZE Zeilen mit contentReference/oaicite (sonst Sätze kaputt)
   - Wandelt literal \n zu echten Zeilen um
*/
function normalizeText(text) {
  ...
  const cleanedLines = String(text)
    ...
    .map(l => l.trim())
    .filter(l => l.length > 0)
    .filter(l => !/contentReference\s*\[|oaicite\s*:|:contentReference/i.test(l));
...
async function loadEntry(id) {
  ...
  box.innerHTML = `
    ...
    <!-- ✅ ACTIONS UNTEN (Social/Kopieren/Drucken) -->
    <div id="entryActions" style="margin-top:26px;border-top:1px solid #e5e5e5;padding-top:14px;">
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        <button data-action="print">🖨️ Drucken</button>
        <button data-action="telegram">Telegram</button>
...
document.addEventListener("click", async (e) => {
  ...
  // Social / Copy / Print (nur wenn entryActions existiert)
  const actionBtn = e.target.closest("#entryActions [data-action]");
  if (actionBtn) {
    const action = actionBtn.getAttribute("data-action");
    const url = location.href;

    // Titel aus H2 (Detailansicht)
...
/* ================= REPORT SUBMIT → SUPABASE.reports (STABIL) ================= */
